...
> 成本估算（tiktoken 精確算）：gpt-4o-mini 每次執行 ≈ $0.0004 USD，每月 22 個交易日 ≈ NT$ 0.3
...
| `watchlist` | — | 自選股代號清單，逗號分隔（例 `XXXX,YYYY,ZZZZ`）。Free 版預留欄位，Paid 版啟用 highlight 功能 |
...
實際每月成本（tiktoken 精確算）：每交易日 1 次 × 22 日 ≈ **$0.009 USD**（約 NT$ 0.3）。儲 $5 大概可以用 1 年以上。
