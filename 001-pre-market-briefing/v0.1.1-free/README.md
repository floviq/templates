...
版本：**v0.1.1-free** ・ 釋出日期：2026-05-03（patch release，see RELEASE-NOTES.md）
...
```
📊 2026-05-04 (週一) 開盤前情報摘要
資料日期：2026-04-30（最近交易日盤後）· 新聞：即時頭條
[AI generated headline] 外資延續觀望，台股震盪整理
...
- **n8n cloud** — n8n.io 註冊免費帳號

差異只在 credential 與環境變數設定位置。本 workflow 在兩個環境都已驗證可用。
...
2. 帳戶儲值至少 $5 USD（每月成本 ≈ $0.022 USD，可用很久）
3. 建議在 `Settings → Limits` 設月度 $1 USD 上限，不會被意外刷爆

> 成本估算：gpt-4o-mini 每次執行 ≈ $0.001 USD，每月 22 個交易日 ≈ $0.022 USD（NT$ 0.7）

#### Telegram Bot token

1. Telegram 搜 `@BotFather` → 對話框送 `/newbot`
2. 照指示取個 bot 名字（會以 `bot` 結尾，例 `floviq_morning_bot`）
3. 拿到一串形如 `123456:ABC-...` 的 token，**這就是 bot token**
...
3. JSON 中找 `chat.id`，是個正整數（例：`123456789`）

**群組**（要分享給家人 / 朋友圈）：

1. 把 bot 加入你要的群組，並設為管理員
2. 群組裡傳任意一句話
...
1. 開 n8n → 左側 Workflows → 右上 `+` → `Import from File`
2. 選 `workflow-free.json`
3. workflow 開啟後，找以下兩個 node 各自連 credential：
...
| 欄位 | 必填 | 說明 |
|---|---|---|
| `telegramChatId` | ✅ | 第 2 步拿到的 chat ID（私訊用正整數，群組用含負號的負整數） |
| `topN` | — | 每段顯示前幾名（預設 5；範圍 3~10 都合理） |
| `watchlist` | — | 自選股代號清單，逗號分隔（例 `XXXX,YYYY,ZZZZ`）。Free 版預留欄位，Paid 版會啟用 highlight 功能 |
| `promptStyle` | — | AI 摘要語氣，三選一：`保守` / `中性` / `活潑`（預設 `中性`） |

### 5. Execute & 啟用排程

1. 右上 **Execute Workflow** 試跑一次
2. 沒意外的話幾秒後 Telegram 會收到一則訊息
3. 確認訊息正常 → 點左上 workflow 名稱旁邊的 **Active** toggle 改 `On`
